...
class WADISegLoader(Dataset):
    def __init__(self, root_path, win_size, step=1, flag="train"):
        ...
        train_data = pd.read_csv(os.path.join(root_path, 'WADI_train.csv'), index_col=0)
        test_data = pd.read_csv(os.path.join(root_path, 'WADI_test.csv'), index_col=0)
...
class UCRSegLoader(Dataset):
    def __init__(self, root_path,file_name, win_size, step=1, flag="train"):
        self.flag = flag
        self.step = step
        self.win_size = win_size
        self.scaler = StandardScaler()
        ...
        time_series, meta_data = pd.DataFrame({name: arr}, index=index), pd.DataFrame(
                {
        ...
        train_data = time_series[meta_data.trainval]
        test_data = time_series[~meta_data.trainval]
        test_labels = meta_data.anomaly[~meta_data.trainval].astype(int)

        train_data = train_data.values
        test_data = test_data.values
        labels = test_labels.values.reshape(len(test_labels.values), 1)
        ...
        # print("test:", self.test.shape, type(self.test))
        # print("test_labels:", self.test_labels.shape, type(self.test_labels))
        # print("train:", self.train.shape, type(self.train))
...
class SWATSegLoader(Dataset):
    def __init__(self, root_path, win_size, step=1, flag="train"):
        ...
        train_data = pd.read_csv(os.path.join(root_path, 'SWaT_train.csv'), index_col=0)
        test_data = pd.read_csv(os.path.join(root_path, 'SWaT_test.csv'), index_col=0)
